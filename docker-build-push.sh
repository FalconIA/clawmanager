#!/usr/bin/env bash
# 构建 Dockerfile 并推送到 Docker Registry 私服
# 用法：
#   ./docker-build-push.sh                      # 自动读取 VERSION 末行
#   ./docker-build-push.sh v2026.5.1            # 手动指定版本标签
#   DOCKER_USER=falconia ./docker-build-push.sh # 覆盖默认用户名
...
IMAGE_NAME="${IMAGE_NAME:-clawmanager}"
